...
//  std::deque<int> value2; 
  ...
  void rrWrite(int value, BinNode* current);           
  void wpre_order(ofstream& out);
  void wrpre_order(BinNode* thyNode, ofstream& out);
  bool symmetry();
  bool rSymmetry(BinNode* right, BinNode* left); 
};
